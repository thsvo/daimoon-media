...
const CampaignBuilderItem = (props) => {
  ...
  //Calculating new prices if target country or Promotional package is added/removed
  useEffect(() => {
    if (
      data.find((o) => o.itemIndex == itemIndex) &&
      region &&
      activeService == 'youtube'
    ) {
      let baseValue = data[itemIndex].values[0].value.baseCostExcl;
      let promotionalCost = 0;

      //calculate basevalue of package
      if (data[itemIndex].values[0].value.promotional) {
        promotionalCost =
          data[itemIndex].values[0].value.promotional[0].promo.cost;
      }

      //finding right basevalue
      const originalValue = baseValue + promotionalCost;

      //Calculating traget cost packages
      let extraCost = 0;

      data[itemIndex].targets.map((item) => {
        const precentage = parseInt(item.cost.replace('%', ''));
        extraCost = extraCost + (originalValue / 100) * precentage;
      });

      const targets = data.find((o) => o.itemIndex == itemIndex).targets;

      const item = data.find((o) => o.itemIndex == itemIndex).values[0];
      if (targets) {
        //Code for storing Region targets
        data[itemIndex].values[0].value = {
          ...item.value,
          targets: targets.length == 0 ? [] : targets,
        };
      }

      data.find((o) => o.itemIndex == itemIndex).values[0].value.cost =
        +originalValue + extraCost;

      setPrice(Math.round((originalValue + extraCost) * 100) / 100);
    }
  }, [region, price]);
  ...
  return (
    <div className={builder.container}>
      <div className={builder.content}>
        {service ? (
          <h3 className={builder.title}>Choose a Campaign</h3>
        ) : (
        ...
        <span
          className={[
            ...
            activeService == 'spotify'
              ...
              : activeService == 'youtube'
              ? builder.youtube
              : activeService == 'soundcloud' && builder.soundcloud,
          ].join(' ')}
        >
          Song: {campaign.track}
        </span>
        {activeService == 'spotify' && data[itemIndex] && (
          <>
            {width <= 768 && (
        ...
        {activeService == 'youtube' && (
          <div className={builder.youtubeTip}>
            <span>
              Tip: Submit non-explicit content. We
              {`'`}ll fully refund videos that get rejected by YouTube.
            </span>
          </div>
        )}

        {Genres && activeService == 'spotify' && (
          <div className={builder.genreContainer}>
            <div className={builder.genres}>
              <div className={builder.genreField}>
                <select
      ...
      <div>
        {width > 768 &&
          (activeService == 'youtube' ? (
            <WorldMap values={data[itemIndex]} region={region} />
          ) : (
            <Graph
              sliderValues={data[itemIndex]}
              activeService={activeService}
                    ...
                    {checkOut == true && unfinishedcampaigns == 'false' ? (
                      ...
                      >
                        {activeService == 'spotify'
                          ...
                          : activeService == 'youtube'
                          ? 'Next Video'
                          : activeService == 'soundcloud' && 'Next Track'}
                      </button>
                    )}
            ...
            {items.map((item, index) => (
              <>
                <div
                  ...
                  className={[
                    styles.image,
                    activeService == 'spotify' && styles.spotify,
                    activeService == 'youtube' && styles.youtube,
...
export default SoundcloudBuilder;
